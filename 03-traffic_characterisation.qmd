...
Overflights account for a small share of the total in both regions.
...
The distribution of air traffic in Brazil confirms that most flights are concentrated in a small number of airports. In 2025, the 10 busiest airports in Brazil handled `r scales::percent(bra_top10_share, accuracy = 1)` of all departing flights, whereas in Europe the top 10 airports account for `r scales::percent(eur_top10_share, accuracy = 1)` of all departures. The spread remains broadly constant up to the 50th rank, narrowing with the top 100 airports — marking `r scales::percent(bra_top100_share, accuracy = 1)` of all departures in Brazil and `r scales::percent(eur_top100_share, accuracy = 1)` in Europe. This reflects the historical development of the European network, where the traditional focus on national hubs resulted in a higher number of major aerodromes interconnecting across the continent.
...
@fig-traffic-distribution shows the distribution of domestic/regional and international departures in 2025. In both
regions, the share of flights operating externally accounts for `r scales::percent(bra_external_share, accuracy = 1)` of total traffic in Brazil and `r scales::percent(eur_external_share, accuracy = 1)` in Europe. The majority of
...
@fig-flow shows the connections between Europe and Brazil in 2025. Individual connections with fewer
than 90 flights per year are aggregated into an "other" group. Traffic from Brazil to Europe accounts for
`r scales::percent(bra_eur_dep_share, accuracy = 0.1)` of the total departures from Brazilian airports. Guarulhos (SBGR) and Lisbon (LPPT) are the main
...
the Middle East, North America, and Africa represent the largest international markets. The chart aligns
Brazil-to-Europe and Europe-to-Brazil traffic on a common counterpart-region row to support comparison. Placeholder
destinations that cannot be mapped to a world region are retained in the canonical data product but
...
@fig-share-south-america provides a closer look at the main country-level external connections. For Brazil,
the focus remains on South America, with just under `r scales::number(bra_sa_deps, accuracy = 1000, scale = 1e-3, suffix = ",000")` flights
operated to adjacent countries in 2025. Argentina and Chile together account for approximately half of
these movements, confirming the strength of the southern cone corridor. For Europe, the comparison
shows the main external country destinations across North America, the Middle East, Africa, and
Asia/Pacific, balancing the number of displayed bars with the Brazilian panel.
...
Figure 3.9 presents the total movements at each study airport in 2025 alongside the variation compared to 2024. While Guarulhos (SBGR) remains the busiest airport in Brazil, its movement volume is comparable to that of the fifth and sixth busiest airports in Europe — Munich (EDDM) and Gatwick (EGKK). This shows that direct comparisons between the top airports in each region may not fully reflect their operational realities, as factors such as the number of runways, network architecture, and traffic density create unique operational complexities in each case.
In Brazil, the largest movement disruption over the period occurred in 2024 at Porto Alegre Airport (SBPA), directly impacted by major flooding in May and remaining closed until October. Many flights were redirected to nearby airports in southern Brazil not included in the scope of this comparison. The increase in movements at Galeão (SBGL) was mainly driven by restrictions applied at Santos Dumont (SBRJ), leading to a reallocation of operations between the two airports — an effect that is also visible in subsequent indicators such as peak day operations.
