...
hlc_raw <- readr::read_csv(
  ...
  dplyr::select(1, dplyr::contains("2023"), dplyr::contains("2024"), dplyr::contains("2025"))
...
In 2025, Europe operated at a density of `r hlc_raw |> dplyr::filter(KPA == "traffic density (non-oceanic flights/km²)") |> dplyr::pull(Europe_2025)` flights/km², significantly higher than Brazil's `r hlc_raw |> dplyr::filter(KPA == "traffic density (non-oceanic flights/km²)") |> dplyr::pull(Brazil_2025)` flights/km². 
...
<!--
MOVED TO NEXT CHAPTER - DELETE HERE WHEN DONE
...
The characteristics of the air navigation systems in Brazil and Europe presented in this chapter highlight both the commonalities and the structural differences that shape operational performance in each region. 
Both systems rely on similar operational concepts, procedures, and supporting technology, yet they differ significantly in their organisational models, workforce dynamics, and traffic density. 
In 2025, Brazil continued to expand its aviation sector, with a growing controller workforce closely aligned with sustained traffic demand, while Europe consolidated its network with a more measured approach to capacity building. 
The traffic density gap — with Europe operating at approximately four times the density of Brazil — remains a fundamental structural feature that underpins many of the performance differences documented throughout this report. 
At the institutional level, both DECEA and EUROCONTROL have continued to deepen their performance monitoring frameworks, with Brazil advancing the calculation of new efficiency indicators such as Vertical Flight Efficiency (VFE) and Horizontal Flight Efficiency (HFE) and strengthening its performance culture through training and annual seminars. 
Together, these developments reinforce the value of the bilateral collaboration and set the analytical foundation for the traffic and performance analysis presented in the following chapters.
